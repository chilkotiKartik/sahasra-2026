...
export default function CCTNSScreen() {
  ...
  const [corpus, setCorpus] = useState<{ totalCrimes: number; totalDistricts: number } | null>(null);

  // Load REAL FIRs + corpus-derived connectors from the backend.
  const loadFirs = useCallback(async () => {
    try {
      const r = await api.get<{ firs: FIRItem[] }>("/api/v2/intel/firs");
      setFirs(r.firs?.length ? r.firs : MOCK_FIRS);
    } catch {
      setFirs(MOCK_FIRS); // offline fallback
    }
  }, []);

  useEffect(() => {
    loadFirs();
    api.get<{ connectors: any[]; summary: any }>("/api/v2/intel/summary")
      .then((r) => { setConnectors(r.connectors || []); setCorpus(r.summary); })
      .catch(() => {});
  }, [loadFirs]);
  ...
  const handleRegisterFIR = async () => {
    ...
    try {
      // Real CRUD: persist the FIR server-side, then reload the list.
      await api.post("/api/v2/intel/firs", {
      ...
      Alert.alert("CCTNS error", e?.message || "Could not register FIR.");
      return;
    }
  ...
  // REAL connectors from the ingested KSP corpus (falls back to static if offline).
  const CONNECTOR_ICONS: Record<string, string> = { cctns: "cloud-done", ipc: "document-text", sll: "shield-checkmark", icjs: "git-network" };
  const interAgencyConnectors = connectors.length
    ? connectors.map((c) => ({ name: c.name, status: c.status === "LIVE" ? "ONLINE" : c.status, icon: CONNECTOR_ICONS[c.key] || "cloud-done", count: `${c.stat}${c.detail ? " · " + c.detail : ""}` }))
    : [
        { name: "CCTNS Karnataka", status: "ONLINE", icon: "cloud-done", count: "Connecting…" },
