...
interface RealZone { id: string; zoneName: string; crimeCount: number; ipcCrimes: number; sllCrimes: number; riskLevel: string; source: string }

export default function HotspotsScreen() {
  const router = useRouter();
  const [selectedSlot, setSelectedSlot] = useState<string>("late_night");
  const [realZones, setRealZones] = useState<RealZone[]>([]);

  // REAL crime density from the ingested KSP corpus (top districts by total crime).
  useEffect(() => {
    api.get<{ realZones: RealZone[] }>("/api/v2/intel/hotspots")
      .then((r) => setRealZones(r.realZones || []))
      .catch(() => {});
  }, []);

  const riskColor = (lvl: string) => (lvl === "Critical" ? "#EF4444" : lvl === "High" ? "#F59E0B" : "#22C55E");
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.scrollContent}>
        {/* REAL KSP corpus: district crime density ranking */}
        {realZones.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>Live KSP District Crime Density ({realZones.length})</Text>
            {realZones.map((z) => (
              <View key={z.id} style={styles.hsCard}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                  <Text style={{ color: "#F0F4FF", fontSize: 15, fontWeight: "700" }}>{z.zoneName}</Text>
                  <View style={{ backgroundColor: riskColor(z.riskLevel) + "22", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8 }}>
                    <Text style={{ color: riskColor(z.riskLevel), fontSize: 10, fontWeight: "800" }}>{z.riskLevel.toUpperCase()}</Text>
                  </View>
                </View>
                <Text style={{ color: "#8892B0", fontSize: 12, marginTop: 4 }}>
                  {z.crimeCount.toLocaleString()} total crimes · IPC {z.ipcCrimes.toLocaleString()} · SLL {z.sllCrimes.toLocaleString()}
                </Text>
                <Text style={{ color: "#64748B", fontSize: 9, marginTop: 2 }}>Source: {z.source}</Text>
              </View>
            ))}
          </>
        )}
